...
func _capture() -> void:
	...
	root.get_node("GameManager")._start_minigame(0, "energy_sort")
	await create_timer(0.7).timeout
	var image := root.get_texture().get_image()
	var error := image.save_png("res://.godot/energy_sort_preview.png")
	print("[EnergySortVisual] %s" % ("PASS" if error == OK else "FAIL"))
